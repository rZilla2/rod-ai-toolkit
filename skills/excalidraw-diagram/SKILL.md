...
### Whitespace — Compact by Default
Keep elements close together. Use **30-50px gaps** between connected elements (arrows between shapes). Only add extra whitespace (100px+) to visually separate unrelated sections or to emphasize the most important element. Avoid large voids between shapes — they make the diagram feel sparse and waste space. The goal is a compact, readable layout that doesn't sacrifice legibility.
...
### Arrow Labels — Centered on the Path
When placing text labels on arrows (e.g., "Yes", "No", "Existing"), position the label at the **midpoint** of the arrow path, offset slightly to one side so it doesn't overlap the line. Do NOT place labels at either end of the arrow — they should sit in the middle of the path unless they explicitly describe something at one end.
